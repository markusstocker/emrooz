...
public class RowKeyUtils {
	...
	private Map<URI, Map<URI, Map<URI, String>>> shaCache;
	private Map<URI, Map<URI, Map<URI, Rollover>>> rolloverCache;
	...
	public RowKeyUtils() {
		...
		this.shaCache = new HashMap<URI, Map<URI, Map<URI, String>>>();
		this.rolloverCache = new HashMap<URI, Map<URI, Map<URI, Rollover>>>();
	}
	...
	private Rollover lookupRollover(URI sensorId, URI propertyId, URI featureId) {
		Map<URI, Map<URI, Rollover>> m1 = rolloverCache.get(sensorId);
	...
	private String lookupShaHex(URI sensorId, URI propertyId, URI featureId) {
		Map<URI, Map<URI, String>> m1 = shaCache.get(sensorId);
	...
	private void cacheShaHex(URI sensorId, URI propertyId, URI featureId, String shahex) {
		Map<URI, Map<URI, String>> m1 = shaCache.get(sensorId);
		
		if (m1 == null) {
			m1 = new HashMap<URI, Map<URI, String>>();
			shaCache.put(sensorId, m1);
		}
